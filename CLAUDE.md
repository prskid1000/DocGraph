...
| `docgraph/docs.py` | URL fetch + HTML→text + chunking + Doc node ingestion. Cursor `@Docs` parity. `add_doc(cfg, url, db=None)` / `remove_doc(cfg, url, db=None)` accept an optional pre-existing writer so the host's `/api/docs/*` routes can reuse the writer they already took (parallel writers would be rejected by Kuzu's exclusive lock). |
| `docgraph/documents.py` | Repo-side document + asset walker (tier 2 + tier 3). Emits text-tier files (`md` / `markdown` / `txt` / `rst` / small CSVs) as `Doc` rows whose `source` is the **logical repo-relative path** (no scheme — disambiguated from `@Docs` URL rows by `is_url_source()` sniffing `http(s)://`). Heavy / binary files (`pdf` / `xlsx` / `png` / `mp4` / `parquet` / fonts / archives / 3D meshes / installers) become `Asset` nodes — metadata only, no content extraction. **Reference extraction is format-aware**: Markdown link/embed syntax + reference-style links + HTML embeds + backtick-wrapped paths; HTML `src=` / `href=` / `data-src=` / `poster=` / `background=` via `HTMLParser`; code paths are matched only inside **quoted string literals** with a real extension (no identifiers / comments / version strings); plain text + RST + CSV use anchored extension regex over the body. Path normalization (forward slashes, lowercased ext, scheme/query/fragment strip) makes Windows-authored repos match cleanly on Linux. Assets are looked up by full path **and** by basename so `<img src="logo.png">` resolves to `/docs/img/logo.png`. CSV size gate at 1 MiB — bigger CSVs route to the asset tier instead of being embedded. Off by default; gated by `cfg.index_documents`. |
...
| `docgraph/server.py` | FastAPI host: web UI + JSON API + SSE `/api/events` + FastMCP mounted at `/mcp` (single port, lifespan chained — uvicorn config must use `lifespan="on"` or the `/mcp` route 500s). `make_app(workspace)` builds a dynamic `RootSlug` enum from the workspace's slugs and threads it through every route as a closed-enum `root` query param. Discovery / admin: `GET /api/roots`, `POST /api/admin/index` (incremental or `{full:true}` reindex via `Workspace.take_writer / release_writer`), `POST /api/admin/clear` (wipe DB + drop `cache.json` + `wiki/`, broadcasts a `reindex_done` SSE), `GET /api/docs/list` / `POST /api/docs/add` / `POST /api/docs/remove` (Cursor `@Docs` parity — same writer-lock dance, reusing the writer DB so we don't fight the host). **No `from __future__ import annotations`** — same enum-evaluation concern as `mcp_tools.py`. |
| `docgraph/ui/index.html` | Single-page graph viewer. Canvas 2D only — Sigma.js / WebGL was removed in 2.1.0 because it was unreliable on first paint. Performance comes from a Web Worker that runs **ForceAtlas2-lite** (linear repulsion, degree-weighted, on a spatial grid) + **label-propagation community detection**. Worker bootstrap is an inline `Blob([WORKER_SRC])`-backed Worker — no esm.sh, no CDN. Render batches edges/nodes by color and viewport-culls in world coords. New tabs: Processes (entry-point → call chains via `/api/processes`) and Wiki (LLM module pages via `/api/wiki/*`). Color-mode toggle (kind / community) lives in the Filters tab. **Multi-root brand pill**: on boot the page hits `/api/roots`; if there's >1 it swaps the static repo label for a `<select>` with one option per slug, threads the active slug into every fetch via `_withRoot()` (appends `?root=<slug>` if not already in the URL), filters the SSE `reindex_done` events by `repo_slug`, and persists the choice to `localStorage` (`?root=` in the URL still wins). Single-root host → label stays static. |
...
- `docs.add_doc(cfg, url, db=None)` — fetch URL with stdlib `urllib`, parse HTML via subclassed `HTMLParser` (no BS4 dep), chunk via `chunk_doc()`, embed, store `Doc` rows. The optional `db` arg lets the host's `/api/docs/add` route reuse the writer it already took; without it `add_doc` opens its own writer.
...
- `URL` and `repo-file` Doc rows share the same table — disambiguated by the leading scheme on `Doc.source` (`http(s)://` = URL tier, anything else = file tier). The cleanup step in `index._index_documents` deletes only the file-tier rows on each pass; URL rows survive a re-index.

## Document + asset indexing (opt-in, tier 2 + tier 3)

Off by default. Activate with `--documents` on `docgraph index` (or `DOCGRAPH_INDEX_DOCUMENTS=1`); `--text-exts` / `--asset-exts` (or the equivalent env vars) override the defaults and **also imply `--documents`** so a user passing only an extension list still enables the pass.

- **Tier 2 — text docs.** `md` / `markdown` / `txt` / `rst` / small CSVs (size-gated to 1 MiB). Read with stdlib only — **no pypdf / openpyxl / python-docx**. Chunk with the same `chunk_doc()` as `@Docs`, embed with the shared `Embedder`, store as `Doc(source=<repo-relative-path>, …)`.
- **Tier 3 — assets.** `pdf` / `xlsx` / `docx` / `png` / `mp4` / `parquet` / fonts / archives / 3D meshes / installers. **No content extraction**, just metadata: `Asset(path, ext, size, mime, repo)`. `mime` is filled from `mimetypes.guess_type` for cheap discoverability.
- **Reference resolution** runs after both tiers are populated. Format-aware string extraction, then a normalized lookup against the registered Asset paths. Hits become `REFERENCES_` edges (the existing edge table now declares `File→Asset / Doc→Asset / Function→Asset / Class→Asset` pairs alongside the original code-tier ones — single edge table, multiple `(FROM, TO)` pairs).
- **Idempotent.** First step of `_index_documents()` does `MATCH (d:Doc) WHERE NOT d.source STARTS WITH 'http(s)://' DETACH DELETE d` so URL-tier `@Docs` rows survive across passes.
- **Stats** reported at the end of the pass: `text_docs / doc_chunks / assets / ref_edges` — also surfaced in the `index_all()` return dict under `documents`.
- **Walking.** `documents.walk_documents(cfg, …)` reuses `Config.is_ignored()` for *directory*-level filtering (so it never descends into `node_modules` / `.git` / `.venv`) but uses `Config.is_user_ignored()` for *file*-level filtering — the universal `*.pdf` / `*.png` / etc. defaults the code walker uses would otherwise pre-empt the asset tier.

## LLM prompt overrides (env-only, telecode-driven)

Two env-var hooks let downstream UIs (telecode) drop in custom prompts without touching code or settings files. Lookup order is **literal text → file path → built-in default**.

- `DOCGRAPH_LLM_PROMPT_DOCSTRING` / `_FILE` — replaces `llm._PROMPT`. Must keep the `{kind}` / `{name}` / `{language}` / `{body}` placeholders. If `.format()` raises `KeyError` (override missing a placeholder), `LLMClient.summarize()` logs a warning and falls back to the default — never crashes the run.
- `DOCGRAPH_LLM_PROMPT_WIKI` / `_FILE` — replaces only the **"Output format" tail** of the wiki prompt (the rendered facts block above it stays). No placeholders required. Read by `wiki._wiki_output_format()`, not by the LLM client.

Both are evaluated lazily on every call (`docstring_prompt_template()` / `wiki_prompt_tail()`), so the telecode tray can flip them between subprocess invocations without restarting the host.
...
- Treating URL `Doc` rows and file `Doc` rows as the same when wiping for the document pass → re-indexing nukes every `@Docs add` URL chunk. The cleanup query must filter by `NOT d.source STARTS WITH 'http(s)://'` so only file-tier rows are dropped.
- Walking the repo with `Config.is_ignored()` at the file level for the document pass → the universal `*.pdf` / `*.png` / etc. exclusions in the code-walker tier swallow every asset before tier 3 sees it. Use `is_user_ignored()` for files in the document walker, `is_ignored()` only for directory pruning.
